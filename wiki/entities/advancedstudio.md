...
**2026-08-23:** the "slicing is deliberately not automated" stance is now half-true: [[obcslicer]] automates slicing headlessly (OrcaSlicer wrapper + provenance slice records) and drives this repo's `/api/propose` seam; the *approval* remains human, exactly as ADR-0002/0009 intended. First fabricated proof: two-material field-banded cube, full audit trail in the jobs ledger. One observation for this repo: a proposal that expires by TTL leaves no jobs row — silent expiry of a guarded decision arguably deserves a ledger entry by this repo's own ledger rationale (ADR-0003).
