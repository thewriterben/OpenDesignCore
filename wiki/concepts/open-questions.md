...
15. ~~**`compare` treats declared instrument accuracy as the whole uncertainty**~~ → **closed 2026-08-24, same day it was opened (ADR-0015).** `--measured` now takes comma-separated repeated readings per dimension; each axis's uncertainty is `max(declared accuracy, observed spread)`, the spread and raw readings are recorded in `odc/comparison/0.3`, and `compensate` re-reads them from the stored record so the widened uncertainty survives into the verdict. Run against the real Z case (readings spread 0.09 mm, mean deviation 0.045 mm), the tool refuses the figure by itself. What remains physical rather than tooling: the Z axis still has no number until an optimised profile can hold a flat top face — the tool now says so instead of a human having to; ~~OpenBuildCore's record stays `axis_calibration: partial` until the reprint~~ → **the reprint happened 2026-08-25 and Z closed.** Tuning the profile took the final-layer spread from 0.08 mm to 0.02 mm, at which point the span was measurable: 21.02 against 21.00, recorded as residual 0 with a 0.19 % floor stated (comparison `cc639d8a24ae`). All three axes verified.

21. ~~**Z has no number**~~ → **closed 2026-08-25; discovered stale 2026-09-11.** Recorded here not because the
    measurement was interesting but because the *claim that it was still open* survived 17 days in ROADMAP and
    in items 14 and 15 above, and a session was about to start re-doing the profile work from scratch. It was
    caught only because a memory of the 08-25 result disagreed with the roadmap and the authoritative record
    (OpenBuildCore `machines.json`) was checked instead of the doc.

    **Second stale status claim found tonight**, after question 6. Both were in files nobody diffs against
    reality: an open-questions list and a roadmap are claims about the present, and neither has anything that
    fails when they go false. The lint operation in this wiki's schema covers wiki pages; it has never been run
    against `ROADMAP.md`. Worth extending, or worth accepting that roadmaps rot and always checking the record
    a roadmap *points at* rather than the roadmap.
