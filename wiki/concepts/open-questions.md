...
15. **`compare` treats declared instrument accuracy as the whole uncertainty, and that is wrong on any axis read off a printed surface** (2026-08-24). Measurement uncertainty is `max(instrument, observed surface spread)`, and the surface usually wins: three readings on the final-layer top face spread 0.08 mm against a 0.02 mm caliper, so the available Z answers ranged from −0.43 % to exactly zero depending on where the jaw landed. Two Z figures were entered and both withdrawn (one transcription error, one never measured); OpenBuildCore's `k2-axes-verified` record now says `axis_calibration: partial` — X and Y verified, Z absent. The tool fix is specified and not yet built: `compare` should take the three readings per axis its own docs request and derive the per-axis spread, which would have refused the Z figure without a human noticing the surface. On the ROADMAP as of 2026-08-24; build before the next measurement run.
